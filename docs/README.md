...
- `5자 이하, 쉼표(,)로 구분`된 자동차 이름 입력 받기
  - 잘못된 값을 입력한 경우, **alert 창 출력**
- 각 자동차의 이름을 쉼표(,)를 기준으로 구분해서 배열로 저장
- '게임 시도할 횟수' 입력 받기 (이동 횟수)
  - 0 이하의 수를 입력한 경우, **alert 창 출력**
- 컴퓨터 0에서 9 사이 랜덤 수 생성
  - 자동차의 이름 숫자 (배열의 길이) 만큼 랜덤 수 반복 생성
  - 자동차 별로 `랜덤 수 > 4` 일경우<br>
    → '-' 출력 (전진) , go 변수 += 1
  - 자동차 별로 `랜덤 수 ≤ 4`일 경우<br>
    → 아무것도 출력하지 않는다. (미전진)
- 랜덤 수 생성 + 출력을 '게임 시도할 횟수'만큼 반복
- 자동차 별로 go 변수의 수 비교
  - go 변수가 가장 높은 자동차의 이름 출력
  - go 변수가 가장 높은 자동차의 이름이 하나 이상일 경우<br>
    → 쉼표(,)를 이용하여 구분된 형태로 차례로 출력
    <br>
