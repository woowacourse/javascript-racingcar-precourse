...
- **게임을 시작하기 전 elements와 버튼 액션을 정의한다.**
  ...
  - 레이싱 결과를 화면에 나타낸다.
- **입력한 값들이 유효한지 검사한다.**
  - 입력받은 차 이름들의 유효성 검사한다.
  - 입력받은 시도할 횟수 유효성 검사한다.
- **각 자동차를 정의한다.**
  - 차 이름으로 초기화 및 어느정도 전진했는지 저장하고 있다.
  - 랜덤으로 0~9사이의 숫자 생성한다.
  - 랜덤 숫자가 4이상일 경우 전진한다.
- **전체 레이싱을 진행한다.**
  - 차 이름 리스트와 횟수로 초기화 및 결과 element 프로퍼티 설정
  - 입력받은 이름들로 각 자동차를 생성한다.
  - 1번의 레이스를 진행한다.
  - 1번의 레이스 결과를 나타낼 element를 생성한다.
  - 우승자들을 가려낸다.
  - 우승자를 결과를 나타낼 element를 생성한다.
  - 입력받은 횟수만큼 레이스를 진행하고 결과를 반환한다.

### 예외 사항

`예외적인 경우에는 alert를 통해 메세지를 전달하고, 저장하지 않는다.`

1. 자동차 이름이 5자를 넘을 경우
2. 자동차 이름을 입력하지 않은 경우
3. 시도할 횟수를 0이하로 입력했을 경우
