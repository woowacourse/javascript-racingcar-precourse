...
- 확인버튼을 눌렀을 때, 입력된 `자동차 이름`들을 검사한다.
  - 입력이 올바르지 않은 경우 경고창을 띄우고, 재입력 받는다.
    - [예외상황] 자동차 이름이 유효하지 않은 문자로만 이루어진 경우
      - 자동차 이름이 공백만으로 이루어진 경우 포함
      - 구분자(,) 뒤에 자동차이름이 입력되지 않은 경우 포함
      - 아예 아무것도 입력되지 않은 경우 포함
...
- 입력받은 자동차들을 대상으로 시도횟수만큼 `게임 한 판`을 반복한다.
  - 게임 한 판 진행방법
...
- 반복을 마친 후 득점이 가장 많은 자동차를 최종 우승자로 출력한다.
  - 이 때 자동차 이름 확인버튼 또는 시도횟수 확인버튼을 누를 경우 실행결과를 지워 새로운 게임을 플레이 할 수 있도록 한다.
