...
## ✔️자동차 경주 게임 회고 (2020.12.02 ~ 2020.12.08)

이번 미션을 진행하면서 내가 가장 집중하고자 했던 부분은 **구현할 기능을 조금 더 세부적으로 나누어 생각해 볼 것**, **그 기능에 따라 모듈을 잘 분리하고, 그 모듈을 담은 폴더를 더 보기 좋게 구조화할 것** 이었다.

### 함수 나누기

- 이번 미션을 진행하며 구현해야 하는 기능에 대해 고민할 때, 기능을 최대한 작게 쪼개려 노력했다. 그리고 매번 개발에 들어가기 전에, 오늘 구현할 기능에 대해 간단히 정리하고 개발에 들어갔다. 하지만 만들다보니 점점 기능이 붙어 여러가지 기능을 하는 함수를 만드는 나의 모습을 볼 수 있었다(물론 나중에 다시 나누긴 했지만🤭..). 처음부터 필요한 기능을 전부 파악해 완벽한 설계를 하는 것은 거의 불가능에 가깝지만, 그럼에도 구현할 기능에 대해 고민할 때 조금 더 시간을 들여 기능을 상세히 나누고 개발하는 연습이 더 필요하다고 생각했다. 1개의 함수가 1개의 기능만을 잘 수행할 수 있게 만들자는 생각을 항시 유념해 프로젝트에 적용해 나가야겠다.

- 저번 주 숫자 야구 게임 미션을 진행할 때에는 구현할 기능의 목록을 작성하며 함수의 이름도 함께 만들었다. 하지만 미션을 진행하며 함수의 이름을 미리 정해두는 등 시작하는 것이 그다지 도움이 되지 않는다는 것을 깨달았다. 함수의 기능을 추가하고 수정하며 이름을 계속 변경했기 때문이다. 이런 상황을 아신 것인지… 미션을 마치고 온 피드백에도 기능 목록을 함수/클래스의 설계와 구현과 같이 너무 상세히 작성하지 않는 것이 좋다고 나와있었다. 덕분에 이번 미션에서 구현할 기능의 세부 사항에 더 집중할 수 있었다.

### 모듈화, 폴더 구조화

- 기능에 따라 모듈을 분리하다보니 폴더를 나누어 모듈을 더 깔끔하게 만들고 싶다는 욕심이 생겼다. 각 모듈이 하는 역할에 맞추어 폴더를 만들어 더 보기 쉽게 구조화하려고 노력했다. 구조화에 정답이 없다고는 하지만, 어떤 식으로 기능을 나누어 모듈화를 하는 것이 좋은지, 그리고 그 모듈을 어떻게 나누어 구조화하는 것이 좋을 지에 대해 계속 고민해나가야겠다.
